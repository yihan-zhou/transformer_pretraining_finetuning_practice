...
def get_name_prediction(model, dataset_object, input_string, train_dataset):
    # x = torch.tensor([dataset_object.stoi[s] for s in input_string], dtype=torch.long)[None,...].to(trainer_obj.device)
    x = torch.tensor([dataset_object.stoi[s] for s in input_string], dtype=torch.long)[None, ...]
...
def evaluate_places(filepath, predicted_places):
  ...
  with open(filepath, encoding='utf-8') as fin:
    ...
    for true_place, predicted_place in zip(true_places, predicted_places):
        if true_place == predicted_place:
            print(f"Correct prediction: {true_place,predicted_place}")
        else:
            print(f"Wrong prediction: {true_place, predicted_place}")
